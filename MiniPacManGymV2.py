# -*- coding: utf-8 -*-
"""MiniPacManGymV2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xhkOAvWKHO9FR2GF3d4ZOfhQAtan_NTH
"""
...
def UpdateLiveGhost(state):
    nextstate = np.copy(state)
    pacman_pos = np.argwhere(state == 1)[0]   # Pacman position
    ghost_pos = np.argwhere(state == 2)[0]   # Ghost position
    nextstate[ghost_pos[0], ghost_pos[1]] = 0 # Remove old ghost

    def dist(pacman_pos,ghost_pos):
      return np.abs(pacman_pos[0]-ghost_pos[0])+np.abs(pacman_pos[1]-ghost_pos[1])

    moves=np.array([[1,0],[0,1],[-1,0],[0,-1]]) #possible ghost movements
    indices=np.arange(4)
    np.random.shuffle(indices)
    moves=moves[indices] #shuffled ghost movements
    options=[]
    for move in moves:
      new_ghost_pos=ghost_pos+move
      if new_ghost_pos[0] == 0 or new_ghost_pos[0] == 5: #don't consider moves that hit walls
        continue
      if new_ghost_pos[1] == 0 or new_ghost_pos[1] == 5:
        continue
      if dist(pacman_pos,new_ghost_pos)>dist(pacman_pos,ghost_pos):
        options=[move]+options #prioritize moves away from pacman
      else:
        options=options+[move] #all else fails, move toward pacman

    ghost_pos=ghost_pos+options[0]
    nextstate[ghost_pos[0], ghost_pos[1]] = 2
    return nextstate
...
def GetReward(state, action):
    ...
    if state[next_pacman_pos[0],next_pacman_pos[1]] == 3:
        return 10
    ...
    live=len(np.argwhere(state == -1))==0

    if not live: # Ghost collision
      old_ghost_pos = np.argwhere(state == -1)[0]
      if (next_pacman_pos[0] == old_ghost_pos[0] and
          next_pacman_pos[1] == old_ghost_pos[1]):
        return -10
    else: #live ghost collision
      live_ghost_pos = np.argwhere(state == 2)[0]
      if (next_pacman_pos[0] == live_ghost_pos[0] and
        next_pacman_pos[1] == live_ghost_pos[1]):
        return 20
...
def MakeBoard(state):
    ...
    #Place live ghost
    if len(np.argwhere(state == 2))!=0:
      ghost_pos = np.argwhere(state == 2)[0]
      board[ghost_pos[0]][ghost_pos[1]] = live_ghost
...
class MiniPacManEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.terminated = (reward == -10 or reward==20)
        self.current_step += 1

        #if powerpill change ghost to live ghost
        if reward==10:
          ghost_pos=np.argwhere(self.state == -1)[0]
          self.state[ghost_pos[0], ghost_pos[1]] = 2

        # If not terminated, move the ghost
        if not self.terminated and not self.frozen_ghost :
          if len(np.argwhere(self.state == -1))==0: #live ghost
            self.state = UpdateLiveGhost(self.state)
          else: #dead ghost
